...
const RadioButton: React.FC<RadioButtonProps> = ({
  ...
  return (
    <label className={outerClassName}>
      ...
      <div
        className={cn(
          `flex flex-col items-center border-2 border-x-[1px] border-text-300 p-3 text-text-200 peer-checked:border-text-100/70 peer-checked:text-text-100`,
          className,
          left ? "rounded-l-lg border-l-2" : "",
          right ? "rounded-r-lg border-r-2" : "",
          noBg ? "" : "peer-checked:bg-black/10"
        )}
        {...rest}
      >
        {children}
